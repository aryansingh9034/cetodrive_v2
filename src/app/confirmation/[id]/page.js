    ...
    const fetchBookingData = async () => {
      try {
        ...
        const bookingResponse = await axios.get(
          `http://3.108.23.172:8002/api/booking/booking/${bookingId}/`
        )
        ...
        // Extract vehicle ID - handle both object and primitive cases
        const vehicleId = bookingData.vehicle?.id || bookingData.vehicle
        
        if (vehicleId) {
          try {
            const vehicleResponse = await axios.get(
              `http://3.108.23.172:8002/api/vehicle/vehicle/${vehicleId}/`
            )
            setVehicle(vehicleResponse.data?.data)
          } catch (vehicleError) {
            console.error('Error fetching vehicle details:', vehicleError)
            // Continue without vehicle details rather than failing completely
          }
        }
      } catch (err) {
              ...
              <div className="flex flex-col md:flex-row items-start md:items-center bg-gradient-to-r from-gray-50 to-blue-50 rounded-2xl p-6">
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center mb-3">
                    ...
                    <span className="text-sm text-gray-600 bg-white px-3 py-1 rounded-full">440+ Reviews</span>
                  </div>
            ...
            <div className="bg-gradient-to-r from-indigo-50 to-blue-50 rounded-2xl p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="text-gray-800 font-medium">{booking.name || 'Not provided'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-1">Email Address</p>
                  <p className="text-gray-800 font-medium">{booking.email || 'Not provided'}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-600 mb-1">Phone Number</p>
                  <p className="text-gray-800 font-medium">{booking.Phone_number || 'Not provided'}</p>
                </div>
                <div>
            ...
            <div className="space-y-4">
              ...
              <button
                onClick={() => {
                  ...
                  const summary = `🎉 Car Rental Booking Confirmed!\n\nBooking ID: #${booking.id}\nVehicle: ${vehicle?.name || 'Vehicle details pending'}\nPickup: ${booking.pick_up_location} on ${formatDate(booking.pick_up_Date)}\nTotal: $${calculateTotalPrice()}\n\nWe're excited for your adventure! 🚗`;
                ...
                className="w-full inline-flex items-center justify-center px-6 py-4 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-xl hover:from-purple-700 hover:to-purple-800 transition-all duration-200 shadow-lg hover:shadow-xl"
              >
